...
public class StudentController {
    ...
    public static void addCourse(Student student, String courseIndexStr)
            throws WrongCourseIndex, AlreadyEnrolled, AlreadyInWaitlist, NoVacancy, MaximumAURegistered, IOException,
            ClashTime {
        // load the courseDB
        ...
        // check if the courseIndex clash time with any enrolled courses
        for (int i = 0; i < courseIndexEnrolled.size(); i++) {
            ...
            for (CourseIndexType classEnrolled : classTypes) {
                ...
                for (CourseIndexType c : chosenCourseIndex.getClassTypes()){
                    if (c.getDay().equalsIgnoreCase(classEnrolled.getDay()) 
                        && isClashedTime(c.getTime(), classEnrolled.getTime())
                        && isClashedWeek(c.getWeek(), classEnrolled.getWeek())) {
                        // if in the same day, check if in the same time
                        throw new ClashTime();
                    }
                }
            }
    ...
    public static void dropCourse(Student student, String courseIndex) {
    ...
    public static void swopCourseIndex(String fromMatricNum, String toMatricNum, String fromCourseIndex, String toCourseIndex) {
    ...
    private static boolean isClashedWeek(ArrayList<Integer> weeks1, ArrayList<Integer> weeks2) {
        for (int w1 : weeks1) {
            if (weeks2.contains(w1)) return true;
        }
        return false;
    }
